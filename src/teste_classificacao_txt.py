...
# %%
new_docs = ['Bolsonaro é alfinetado em desfile de escolas de samba no Rio',
            'Fantasias indígenas e com referências à África levantam debate no Carnaval',
            'Megablocos concentram público e patrocínio e encarecem carnaval de SP',
            ...
            'Brasileiros sequenciam genoma do coronavírus em 48 horas'
            'Brasil confirma segundo caso de coronavírus, também em São Paulo',
            'Gripe matou 37 pessoas no Distrito Federal em 2019',
            'Brasil tem 252 casos suspeitos de coronavírus e dois confirmados',
            'Coronavírus: estados estudam suspender cirurgias e tratamentos']
...
print(pd.DataFrame(data={'text': X_test, 'label': y_test,
                         'predicted': y_test_pred}, columns=['text', 'label', 'predicted']))
print(pd.DataFrame(data={'text': new_docs,
                         'predicted': y_new_pred}, columns=['text', 'predicted']))
