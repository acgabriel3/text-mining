...
load("/home/sbrt/dados/sbrt/dados/Rdata/sbrt_vis_final_respostas40.RData")
...
load("/home/sbrt/dados/sbrt/dados/Rdata/sbrt_vis_20_dossies.RData")


#load(paste0(dir_app,"relatorio_resposta/base/sbrt_vis_010_dossies.RData"))
...
```{r include=FALSE}
#obervers para o checkbox do CNAE Macro
  #output$res1 <- renderPrint({
  #  input$check1
  #})
...
best_model<-train_model(instance_mallet,n_topics = 1, n_iters = 1,n_max_iters = 1, seed = 12345)
...
#obervers para o checkbox das instituicoes
  ...
  observeEvent(input$a2z, {
    updateCheckboxGroupInput(
      session = session, inputId = "check5", choices = sort(unique(dossies_topic$INST)), selected = input$check5
    )
  })
  # Sorting desc
  observeEvent(input$z2a, {
    updateCheckboxGroupInput(
      session = session, inputId = "check5", choices = rev(sort(unique(dossies_topic$INST))), selected = input$check5
    )
  })
  ...
  # Select all / Unselect all
  observeEvent(input$all_inst_dossie, {
    if (is.null(input$check5)) {
      updateCheckboxGroupInput(
        session = session, inputId = "check5", selected = sort(unique(dossies_topic$INST))
      )
    } else {
      updateCheckboxGroupInput(
...
#obervers para o checkbox do CNAE Macro
  ...
  observeEvent(input$a2z, {
    updateCheckboxGroupInput(
      session = session, inputId = "check6", choices = sort(unique(dossies_topic$CNAE_MACRO)), selected = input$check6
    )
  })
  # Sorting desc
  observeEvent(input$z2a, {
    updateCheckboxGroupInput(
      session = session, inputId = "check6", choices = rev(sort(unique(dossies_topic$CNAE_MACRO))), selected = input$check6
    )
  })
  ...
  # Select all / Unselect all
  observeEvent(input$all_cnaemacro_dossie, {
    if (is.null(input$check6)) {
      updateCheckboxGroupInput(
        session = session, inputId = "check6", selected =  sort(unique(dossies_topic$CNAE_MACRO))
      )
    } else {
      updateCheckboxGroupInput(
...
#selector de tópicos
tags$h4("Tópico"),
selectInput("topicnum_dossies",
label = "Selecione um tópico:", 
choices=topics_selector_dossies,
selected = topics_selector_dossies[[1]], 
width = "100%"),
...
actionButton(
inputId = "all_inst_dossie",
label ="Todos/Nenhum"),

checkboxGroupInput(inputId = "check5",
label = "Selecione",
choices =sort(unique(dossies_topic$INST)),
selected = sort(unique(dossies_topic$INST))
)
),
...
actionButton(
inputId = "all_cnaemacro_dossie",
label ="Todos/Nenhum"),

checkboxGroupInput(inputId = "check6",
label = "Selecione",
choices =sort(unique(dossies_topic$CNAE_MACRO)),
selected = sort(unique(dossies_topic$CNAE_MACRO))
)
),
...
#selector de intervalo de anos
sliderInput(
"ano_dossies", "Selecione o intervalo de anos:",
min = min(periodo_dossies),
max = max(periodo_dossies),
value = c(min(periodo_dossies),max(periodo_dossies)),
sep="")
...
```{r include=FALSE}
#inputs
num_dossies <- reactive(as.integer(input$topicnum_dossies))
insti_dossies<-reactive(input$check5)
cnaemacro_dossies<-reactive(input$check6)
ano_inicio_dossies<-reactive(as.numeric(input$ano_dossies[1]))
ano_fim_dossies<-reactive(as.numeric(input$ano_dossies[2]))
...
```{r}
renderValueBox(
valueBox(as.character(dossies_n()), icon = "fa-file-alt"))
```


### Proporção do total de dossiês do tópico

```{r}
porcentagem_dossies<-reactive({round(dossies_n()/n_dossies_topico()*100,digits = 2)})
renderValueBox(
valueBox(as.character(paste(porcentagem_dossies(), "%")), icon = "fa-chart-bar"))
```
...
```{r}
dossies_tbl<-reactive({dossies_topic%>%
  filter(topic==num_dossies())%>%
  filter(INST %in% insti_dossies())%>%
  filter(CNAE_MACRO %in% cnaemacro_dossies())%>%
  filter(periodo>=ano_inicio_dossies())%>%
  filter(periodo<=ano_fim_dossies())%>%
  ungroup()%>%
  select(-topic,-periodo)
})

sel_topic_dossies<-reactive({dossies_topic%>%
  filter(topic==num_dossies())})

n_dossies_topico<-reactive({dossies_topic%>%
  filter(topic==num_dossies())%>%nrow()})
```
...
columns=c("gamma"),
digits=3)%>%
formatStyle(c("document", "INST",
              "CNAE_MACRO","CNAE_DESCR", "TITULO_DT",
              "DESCR_DOSSIE", "DATA_CRIACAO", "gamma"),
              `text-align` = "center")
})
```
...
```{r include=FALSE}
termos_dossies<-reactive({
  
if(identical(sel_topic_dossies()$document,dossies_tbl()$document)){
  terms_sbrt_dossies%>%
    filter(topic==num_dossies())%>%
    select(-topic)%>%
    filter(beta>0.00015)
...
dossies_base<-data.frame(id=list.files(dosies_dir))
dossies_selecionados<-paste0(dossies_tbl()$document,".txt")

docs_interesse<-filter(dossies_base, id %in% dossies_selecionados)

docs_interesse<-paste0(dosies_dir,docs_interesse$id)
...
dendrograma_docs %>% 
  ...
  set("branches_lwd", 2)%>%
  plot(horiz=T, axes=F, main = "Respostas Técnicas")
...
Dossiês Técnicos {data-navmenu="Dendrogramas"}
====================================================================================
...
dendrograma_docs_dossies %>% 
  ...
  set("hang_leaves",0.05)%>%
  set("branches_lwd", 2)%>%
  plot(horiz=T, axes=F, main = "Dossiês Técnicos")
...
<iframe width = "100%" height = "100%" frameborder="0" src="http://34.73.23.127/RT/index.html"/>
...
<iframe width = "100%" height = "100%" frameborder="0" src="http://34.73.23.127/DT/index.html"/>
