...
selectInput("categ", label = h3("Selecione uma ou mais categorias"), 
    choices = levels(respostas_topic$`Categoria atual do SBRT`),
    multiple = TRUE)
```
...
```{r}
fluidPage(
  tags$h5("Selecione uma ou mais UFs"),
  #br(),
  fluidRow(
    column(
      width = 6,
...
```{r}
fluidPage(
  tags$h5("Selecione uma ou mais instituicao"),
  #br(),
  fluidRow(
    column(
      width = 6,
...
```{r}
basicPage(sliderInput("ano", "Ano",min = min(periodo), max = max(periodo), value = c(min(periodo),max(periodo)),sep=""),textOutput("SliderText2"))

my_range2 <- reactive({
    cbind(input$ano[1],input$ano[2])
})
output$SliderText2 <- renderText({my_range2()})
```

```{r}

basicPage(sliderInput("range", "Age:",min = 0, max = 100, value = c(0,100)),textOutput("SliderText"))
  my_range <- reactive({
...
respostas_topic%>%
  ...
  ungroup()%>%
  select(-topic, -AUTOR_QT,-COD_QT,-CNAE_SUB_RT)#,digits=3
)
```
