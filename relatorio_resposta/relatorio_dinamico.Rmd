...
fluidPage(
  ...
  tags$h4("Tópico:"),
  #br(),
 selectInput("topicnum", label = "", 
    choices=topics_selector,
    #choices=list("1"=1,"2"=2),

    selected = topics_selector[[1]], width = "100%"))
```
...
fluidPage(
  ...
  fluidRow(
    column(
      width = 6,
      dropdownButton(
        label = "Estados", status = "default", width = 100,
...
fluidPage(
  ...
  fluidRow(
    ...
    column(
      width = 6,
      dropdownButton(
        label = "Instituições", status = "default", width = 100,
        actionButton(inputId = "a2z", label = "A - Z", icon = icon("sort-alpha-asc")),
        actionButton(inputId = "z2a", label = "Z - A", icon = icon("sort-alpha-desc")),
        br(),
...
fluidPage(
  br(),
  sliderInput("ano", "Ano:",min = min(periodo), max = max(periodo), value = c(min(periodo),max(periodo)),sep=""))
